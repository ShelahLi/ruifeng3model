...
#date:20170314
start_time <- Sys.time()
cat(paste(start_time,"开始执行程序：Star.R",sep = "***"),"\n")
...
kdbuser <- kArgs[5]
kdbpasswd <- kArgs[6]
pathout <- paste("/etl/etldata/output/",ktoday,"/star",sep="")
...
variable_data <- read.table("/etl/etldata/script/conf/star_variable_list.txt", 
...
tab0 <- txSqlQuery(sqlConnection = sqlCon, query = paste( "select star_down_f,",variables," from ",kdbname,".idv_cst_smy_star where star_down_f='0'",sep=""))
tab1 <- txSqlQuery(sqlConnection = sqlCon, query = paste( "select star_down_f,",variables," from ",kdbname,".idv_cst_smy_star where star_down_f='1'",sep=""))
num1<-txCount(tab1)
...
material_table <- txSqlQuery(sqlConnection = sqlCon, query = paste( "select idv_crm_cst_id,",variables," from ",kdbname,".idv_cst_smy_star_fc",sep="") )
...
write.table(list_all_0.5, paste( pathout,"/StarList.csv",sep=""),
            row.names = FALSE,col.names = TRUE, quote = FALSE,sep=",")
cat(paste(Sys.time(),"***成功生成客户名单,并导出至路径",pathout,"/StarList.csv\n", sep = ""))

end_time <- Sys.time()
cat(as.character(end_time),"***成功执行程序：Star.R\n","总耗时",as.numeric(difftime(end_time,start_time,units = "mins")),"minutes\n")
