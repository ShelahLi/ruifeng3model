...
today1=`date -d "${today1} -2 month " +%Y-%m-%d`
...
Rscript --slave ${indir}/Star_optimize.R ${today} ${IP} ${PORT} ${DBNAME} ${DBUSER} ${DBPASSWD}  >${logdir}/Star_optimize.log



[ $? -ne 0 ] && echo "######### 模型优化错误,请查看日志文件 ${logdir}/Star_optimize.log#########" && exit -1
echo "######### 模型优化成功，请查看所优化的变量列表 ${ETCDIR}/star_variable_list.txt 若有需要可以手工调整 #########"
