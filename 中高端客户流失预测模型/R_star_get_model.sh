...
[ ! -f ${ETCDIR}/star_variable_list.txt ] && echo "######### 模型变量文件不存在，请先运行 R_ahead_star_get_model.sh 或手工设置变量文件 #########" && exit -1
...
today1=`date -d "${today1} -1 month " +%Y-%m-%d`
today1=`date -d "${today1} -1 day " +%Y-%m-%d`
lst=\'`date -d "${today1} " +%Y-%m-%d`\'
num=`${TDH} -e "select count(*) from ${DBNAME}.idv_cst_smy_star where smy_dt=${lst}" | grep -v "_c0" | grep "|" |  sed 's/|/''/g' | awk '{ print int($1)}'`
[ ${num} -eq 0 ] && echo "idv_cst_smy_star里没有更新数据" && exit -1
echo "idv_cst_smy_star里更新数据量 ${num}" 


indir=/etl/etldata/script/R
outdir=/etl/etldata/output/${today}/star
...
Rscript  ${indir}/Star.R ${today} ${IP} ${PORT} ${DBNAME} ${DBUSER} ${DBPASSWD}  >${logdir}/Star.log



[ $? -ne 0 ] && echo "######### 名单预测失败，请查看日志 ${logdir}/Star.log#########" && exit -1
...
echo "######### 名单预测成功，请在路径 ${outdir} 下查看生成的名单 StarList.csv #########"
exit 0
