...
tab <- txSqlQuery(sqlConnection = sqlCon,
                  query = paste("select * from ",kdbname,".idv_cst_smy_ln ",sep=""))
All <- txCollect(tab)
...
All <- All[,-c(which(names(All)=="smy_dt"),
                     ...
                     which(names(All)=="ip_cr_rsk_rtg_cd"),
                     which(names(All)=="cr_lmt"),
                     ...
                     which(names(All)=="cst_cgy_id"),
                     which(names(All)=="fms_prd_risk"),
                     which(names(All)=="ln_cst_f"),
                     which(names(All)=="cur_ln_bal_amt"),
                     which(names(All)=="mth_acml_ln_rpymt_amt"),
                     which(names(All)=="l3m_acml_ln_rpymt_amt"),
                     which(names(All)=="l6m_acml_ln_rpymt_amt"),
                     which(names(All)=="mth_ln_avg_bal_amt"),
                     which(names(All)=="l3m_ln_avg_bal_amt"),
                     which(names(All)=="l6m_ln_avg_bal_amt"),
                     which(names(All)=="mth_avg_ln_dep_rat"),
                     which(names(All)=="l3m_avg_ln_dep_rat"),
                     which(names(All)=="l6m_avg_ln_dep_rat"),
                     which(names(All)=="mth_loan_acc_bal_k"))]  
...
All_final <- FDataPretreatment(All,"ln_act_f",0.75)
All_final$ln_act_f<-factor(All_final$ln_act_f)
...
cat(paste(Sys.time(),"开始选变量... 预计耗时",sep = "***"),length(list_ordinal)*15,"seconds","\n")
...
btime = proc.time()
  train_part<-train[c(list_ordinal[1:i],"ln_act_f")]
  rf_model<-randomForest(ln_act_f~. , data =train_part, ntree=100 ,importance=TRUE)
  
  
  model_pre <- predict( rf_model,newdata=valid,type = "prob") 
    
  roc_list <- roc(valid$ln_act_f, model_pre[,2])
  roc_list_huice <- roc(train_part$ln_act_f ,rf_model$votes[,2])
...
write.table(list_original_order_part,
            "/etl/etldata/script/conf/ln_variable_list.txt",
            col.names = F, row.names = F, sep=",")
cat(paste(Sys.time(),"成功保存优化结果，即最优变量清单至路径 /etl/etldata/script/conf/ln_variable_list.txt",sep = "***"),"\n")
...
optimize_result[which(optimize_result$model=="ln"),c(2,3)] <- AUC_value[which.max(AUC_value$AUC),c(3,6)]
